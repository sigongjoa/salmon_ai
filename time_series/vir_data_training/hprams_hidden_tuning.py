...
def hidden_tuning(hidden_size):
    text_path = 'config_hidden.txt'
    config = ModelConfig()
    ...
    save_path = f"results_tuning/hidden/trail_{config.trail_num}/{config.training_count}"
    ...
    logger = TensorBoardLogger(f"lightning_logs/trail_config_hparams_hidden_head_tuning/{config.training_count}")
...
for hidden_size in [4,8,12,16,20,24,28,32,36,40,56,62,70,76,84,90]:
    hidden_tuning(hidden_size)
